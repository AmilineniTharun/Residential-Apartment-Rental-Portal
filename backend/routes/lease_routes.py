    ...
    try:
        cur.execute("""
            SELECT b.id as booking_id, b.status, b.booking_date,
                   un.unit_number, un.floor, un.price as rent_amount,
...
def download_lease(booking_id):
    ...
    try:
        ...
        # Fetch full data with all details for the new PDF
        cur.execute("""
            SELECT u.email as user_email, u.full_name,
                   un.unit_number, un.floor, un.price as rent_amount, un.security_deposit as deposit_amount, un.bhk, un.location as unit_location,
                   t.name as tower_name, t.id as tower_id, t.area, t.street, t.city, t.state,
        ...
        # Add placeholders for missing tenant fields
        data['phone'] = None 
        data['gov_id'] = None
        
        # Check if lease exists in DB for start/end dates
        ...
        # Fetch amenity names for this unit
        cur.execute("""
            SELECT a.name 
            FROM unit_amenities ua 
            JOIN amenities a ON ua.amenity_id = a.id 
            WHERE ua.unit_id = (SELECT unit_id FROM bookings WHERE id = %s)
        """, (booking_id,))
        amenities_rows = cur.fetchall()
        data['amenities'] = [row['name'] for row in amenities_rows]
...
def submit_lease_request():
    ...
    reason = data.get('reason')               # string, optional
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO lease_requests (booking_id, request_type, extend_days, new_end_date, reason)
            VALUES (%s, %s, %s, %s, %s) RETURNING id
        """, (booking_id, request_type, extend_days if request_type == 'extend' else None, 
              new_end_date if request_type in ('extend', 'vacate') else None, reason))
    ...
    try:
        cur.execute("""
            SELECT lr.id, lr.booking_id, lr.request_type, lr.extend_days, lr.new_end_date,
                   lr.status, lr.admin_note, lr.requested_at, lr.resolved_at, lr.reason,
...
def admin_approve_lease_request(req_id):
    ...
    try:
        ...
        if req['request_type'] == 'terminate':
            ...
            # 2. Update the booking status to 'vacated'
...
def admin_terminate_lease(booking_id):
    ...
    try:
        ...
        """, (booking_id, end_date))
        # Direct termination also leads to 'vacated' status
        cur.execute("UPDATE bookings SET status = 'vacated' WHERE id = %s", (booking_id,))
