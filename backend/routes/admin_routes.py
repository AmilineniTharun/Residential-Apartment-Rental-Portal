...
def create_tower():
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO towers (name, floors, description, image_url, state, city, area, street, units_per_floor) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",
...
def create_flat():
    ...
    try:
        ...
        if images:
            upload_dir = os.path.join(request.environ.get('werkzeug.server.shutdown', lambda: None).__globals__.get('app', request.current_app).root_path if hasattr(request, 'current_app') else request.environ['werkzeug.server.shutdown'].__globals__['app'].root_path if 'werkzeug.server.shutdown' in request.environ else os.getcwd(), 'static', 'uploads')
            os.makedirs(upload_dir, exist_ok=True)
            for file_idx, file in enumerate(images):
                if file and file.filename:
                    # Sanitize and append timestamp for uniqueness
                    ...
                    public_url = f"http://localhost:5000/static/uploads/{unique_filename}"
                    cur.execute(
                    ...
                    if file_idx == default_idx:
                        cur.execute("UPDATE units SET image_url = %s WHERE id = %s", (public_url, unit_id))
    ...
    try:
        ...
        if valid_images:
            ...
            for file_idx, file in enumerate(valid_images):
                ...
                if file_idx == default_idx:
                    cur.execute("UPDATE units SET image_url = %s WHERE id = %s", (public_url, id))
    ...
    try:
        cur.execute("""
            SELECT b.id, b.status, b.booking_date,
                   u.unit_number, u.price, u.image_url,
...
def approve_booking(id):
    ...
    try:
        ...
        # Update booking
        cur.execute("UPDATE bookings SET status = 'approved' WHERE id = %s", (id,))
        # Update unit
        cur.execute("UPDATE units SET status = 'rented' WHERE id = %s", (booking['unit_id'],))
        ...
        message = f"Congratulations!\n\nYour booking request for Unit {booking['unit_number']} has been approved by the admin. Please log in to your dashboard to view lease details.\n\nRegards,\nRental Portal Team"
...
def get_reports():
    ...
    try:
        ...
        cur.execute("""
            SELECT SUM(price) as expected_monthly_revenue 
            FROM units 
            WHERE status = 'rented'
        """)
        revenue = cur.fetchone()
        
        # Total Unique Tenants (Users with approved bookings)
        cur.execute("""
            SELECT COUNT(DISTINCT user_id) as total_tenants 
            FROM bookings 
