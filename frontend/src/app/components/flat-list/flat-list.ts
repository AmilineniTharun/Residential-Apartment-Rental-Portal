...
export class FlatList implements OnInit {
  ...
  viewDetails(flat: Flat): void {
    ...
    this.loadImages(flat.id);
    document.body.style.overflow = 'hidden';
    // Also fetch full detail from individual endpoint to get floor/wing/location
  ...
  loadImages(unitId: number): void {
    this.flatService.getFlatImages(unitId).subscribe({
      next: (res) => {
        ...
        } else if (this.selectedFlat?.image_url) {
          // Fallback to the single image URL if the images array is empty
