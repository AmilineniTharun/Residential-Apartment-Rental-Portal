...
export class FlatService {
    ...
    getReviews(unitId: number): Observable<{ reviews: Review[] }> {
        return this.http.get<{ reviews: Review[] }>(`http://localhost:5000/api/reviews/${unitId}`);
    }
    ...
    postReview(unitId: number, rating: number, comment: string): Observable<any> {
        ...
        return this.http.post('http://localhost:5000/api/reviews/', {
            unit_id: unitId,
            rating: rating,
            comment: comment
        }, { headers });
    ...
    getRecommendations(): Observable<any> {
        ...
        return this.http.get<any>('http://127.0.0.1:5000/api/recommendations/', { headers });
    }
}
